<!doctype html>
<html lang="en">
    <?php $page = 'home'; include 'head.php'; ?>
    <body>
        <div class="wrapper">
            <!--Header Start-->
            <?php include'menu.php'; ?>
            <div id="search">
                <button type="button" class="close">×</button>
                <form class="search-overlay-form">
                    <input type="search" value="" placeholder="type keyword(s) here" />
                    <button type="submit" class="btn btn-primary"><i class="fas fa-search"></i></button>
                </form>
            </div>
            <!--Header End-->
            <!--Inner Header Start-->
            <section class="wf100 p100 inner-header" style="background: url(images/safety-products/Untitled-2.jpg) no-repeat;">
                <div class="container">
                    <h1>Industrial Safety Products</h1>
                    <ul>
                        <li><a href="#">Home</a></li>
                        <li><a href="#"> Products </a></li>
                        <li><a href="#">Safety Products</a></li>
                    </ul>
                </div>
            </section>
            <!--Inner Header End--> 
               <section class="wf100 p80">
                <div class="container">
                    
                    <div class="row">
                        <div class="col-md-12">
                          <h5>FINEST STANDARD IN SAFETY SHOES</h5>
                    </div>
                    </div>
                    
                    <div class="row">
                        <div class="col-md-12">
                            <img src="images/safety-products/commando.jpg" alt="" />  
                        </div>
                    </div><br><br>

                    <div class="row">
                        <div class="col-md-12">
                            <h3>A TRADITION OF EXCELLENCE</h3>
                            <span>VPN COMMODITIES Pte Ltd Singapore is a well-established certified organization active in manufacturing & trading business of different kinds of safely footwear.The company has a strong quality base that is being continously developed to acheive a higher level of perfection in design and workmanship.The empahsis since the very beginning has been to offer "great product at great prices."</span>>
                        </div>
                    </div><br>
                    <div class="row">
                        <div class="col-md-12">
                            <h3>OUR MOTTO</h3>
                            <span>"TO exceed customer expectations through Innovation,Continous Improvement,Quality,Cost and Delivery".</span>
                        </div>
                    </div><br>
                    
                    <div class="row">
                        <div class="col-md-8">
                            <div class="legacy-trust">
                                <h5>A LEGACY OF TRUST</h5>
                                <span>'COMMANDO' Safety Gear,from the house of VPN, is one of the most trusted safety industrial shoe brands in Singapore made on Direct Injection Process on Desma Machines from Germany.'COMMANDO' is certified by EU NORMS(EN ISO 20345:2011)from PFI Germany/Intertek UK. It has become a part of contemporary work culture in Aviation,Chemical Industry,Construction Business,Defence Services,Heavy Engineering,Hotels,Mining in National and International markets.</span>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <img src="images/safety-products/trust.jpg" alt="" />
                        </div>
                    </div>
                    
                    <div class="row">
                        <div class="col-md-8">
                            <div class="legacy-trust">
                                <h5>A CAPABILITY TO CUSTOMIZE</h5>
                                <span>VPN has not only its own unique styling but also has the capability to react to special market requirements & can customize shoes as per the specification of the customer at a very competitive price and on time. With a firm commitment and belief,we can create products matching your interests.</span>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <img src="images/safety-products/capability-to-customize.jpg" alt="" />
                        </div>
                    </div><br>
                    
                    <div class="row">
                        <div class="col-md-12">
                          <h5>A RANGE FOR DIVERSE SEGMENTS</h5>
                           <span>Our Commando Safety Gear catres to diverse requirements of people in different areas of activity...airports,chemical industry,construction business,defence services,heavy engineering,hotel,mining or railways...ensuring they go about their business with absolute safety and confidence.</span>
                         </div>
                    </div>
                    
                    <div class="row">
                        <div class="col-md-12">
                            <img src="images/safety-products/range-of-diverse-segments1.jpg" alt="" />  
                        </div>
                    </div>
                </div>
            </section>            
            
                            <!--Blog Post End-->
                           <!--Blog Post End-->
                        <section class="wf100 p80 projects">
            <div class="projects-grid-two">
               <div class="container">
                   <h4 align="left">Product Range</h4>
                  <div class="row">
                     <!--Project box Start-->
                     <div class="col-md-4 col-sm-6">
                        <div class="pro-box">
                            <img src="images/safety-products/shoee1.jpg" alt="">
                           <h5>EN ISO 20345:2011</h5>
                           <div class="pro-hover">
                              <h6>ART NO  - COMMANDO-123</h6>
                              <span><br>ART NO.   - COMMANDO-123<br>
                                    STANDARD  - S1P<br>
                                    UPPER     - Buff Barton<br>
                                    COLOUR    - Black/Ten<br>
                                    LINING    - Cambrella<br>
                                    TOE       - Steel<br>
                                    SOCKS     - Moulded Antistatic<br>
                                    INSOLE    - EVA Kevlar<br>
                                    SOLE      - Double/Single Density
                              
                              </span>
                              
                           </div>
                        </div>
                     </div>
                     <!--Project box End-->
                     <!--Project box Start-->
                     <div class="col-md-4 col-sm-6">
                        <div class="pro-box">
                            <img src="images/safety-products/shoee2.jpg" alt="">
                           <h5>EN ISO 20345:2011</h5>
                           <div class="pro-hover">
                              <h6>ART NO - COMMANDO-320</h6>
                              <span><br>ART NO.   - COMMANDO-123<br>
                                    STANDARD  - S1P<br>
                                    UPPER     - Buff Barton<br>
                                    COLOUR    - Black<br>
                                    LINING    - Cambrella<br>
                                    TOE       - Steel<br>
                                    SOCKS     - Moulded Antistatic<br>
                                    INSOLE    - Kevlar<br>
                                    SOLE      - Double/Single Density
                              
                              </span>
                           </div>
                        </div>
                     </div>
                     <!--Project box End--> 
                     <!--Project box Start-->
                     <div class="col-md-4 col-sm-6">
                        <div class="pro-box">
                            <img src="images/safety-products/shoe3.jpg" alt="">
                           <h5>EN ISO 20345:2011</h5>
                           <div class="pro-hover">
                               <h6>ART NO.   - COMMANDO-320</h6>
                               <span><br>ART NO.   - COMMANDO-320<br>
                                         STANDARD  - S1P<br>
                                         UPPER     - Buff Barton<br>
                                         COLOUR    - Brown<br>
                                         LINING    - Cambrella<br>
                                         TOE       - Steel<br>
                                         SOCKS     - Moulded Antistatic<br>
                                         INSOLE    - Kevlar<br>
                                         SOLE      - Double/Single Density
                              
                              </span>
                           </div>
                        </div>
                     </div>
                     <!--Project box End--> 
                     <!--Project box Start-->
                     <div class="col-md-4 col-sm-6">
                        <div class="pro-box">
                            <img src="images/safety-products/shoee4.jpg" alt="">
                           <h5>EN ISO 20345:2011</h5>
                           <div class="pro-hover">
                               <h6>ART NO - COMMANDO-334</h6>
                               <span><br>ART NO.   - COMMANDO-334<br>
                                    STANDARD  - S1P<br>
                                    UPPER     - Buff Barton<br>
                                    COLOUR    - Black/Ten<br>
                                    LINING    - Cambrella<br>
                                    TOE       - Steel<br>
                                    SOCKS     - Moulded Antistatic<br>
                                    INSOLE    - Kevlar<br>
                                    SOLE      - Double/Single Density
                              
                              </span>
                           </div>
                        </div>
                     </div>
                     <!--Project box End--> 
                     <!--Project box Start-->
                     <div class="col-md-4 col-sm-6">
                        <div class="pro-box">
                            <img src="images/safety-products/shoee5.jpg" alt="">
                           <h5>EN ISO 20345:2011</h5>
                           <div class="pro-hover">
                              <h6>ART NO - COMMANDO-01</h6>
                              <span><br>ART NO.   - COMMANDO-01<br>
                                        STANDARD  - S1P<br>
                                        UPPER     - Buff Barton<br>
                                        COLOUR    - Black<br>
                                        LINING    - Cambrella<br>
                                        TOE       - Steel<br>
                                        SOCKS     - Moulded Antistatic<br>
                                        INSOLE    - Kevlar<br>
                                        SOLE      - Double/Single Density
                              
                              </span>
                           </div>
                        </div>
                     </div>
                     <!--Project box End--> 
                     <!--Project box Start-->
                     <div class="col-md-4 col-sm-6">
                        <div class="pro-box">
                            <img src="images/safety-products/shoe6.jpg" alt="">
                           <h5>EN ISO 20345:2011</h5>
                           <div class="pro-hover">
                              <h6>ART NO.   - COMMANDO-333</h6>
                              <span><br>ART NO.   - COMMANDO-333<br>
                                        STANDARD  - S1P<br>
                                        UPPER     - Buff Barton<br>
                                        COLOUR    - Ten<br>
                                        LINING    - Cambrella<br>
                                        TOE       - Steel<br>
                                        SOCKS     - Moulded Antistatic<br>
                                        INSOLE    - Kevlar<br>
                                        SOLE      - Double/Single Density
                              
                              </span> 
                           </div>
                        </div>
                     </div>
                     <!--Project box End--> 
                     <!--Project box Start-->
                     <div class="col-md-4 col-sm-6">
                        <div class="pro-box">
                            <img src="images/safety-products/shoee7.jpg" alt="">
                           <h5>EN ISO 20345:2011</h5>
                           <div class="pro-hover">
                              <h6>ART NO - COMMANDO-319</h6>
                              <span><br>ART NO.   - COMMANDO-319<br>
                                    STANDARD  - S1P<br>
                                    UPPER     - Buff Barton<br>
                                    COLOUR    - Black/Ten<br>
                                    LINING    - Cambrella<br>
                                    TOE       - Steel<br>
                                    SOCKS     - Moulded Antistatic<br>
                                    INSOLE    - EVA Kevlar<br>
                                    SOLE      - Double/Single Density
                              
                              </span>
                           </div>
                        </div>
                     </div>
                     <!--Project box End--> 
                     <!--Project box Start-->
                     <div class="col-md-4 col-sm-6">
                        <div class="pro-box">
                            <img src="images/safety-products/shoee8.jpg" alt="">
                           <h5>EN ISO 20345:2011</h5>
                           <div class="pro-hover">
                              <h6>ART NO.   - COMMANDO-319</h6>
                              <span><br>ART NO.   - COMMANDO-319<br>
                                    STANDARD  - S1P<br>
                                    UPPER     - Buff Barton<br>
                                    COLOUR    - Brown<br>
                                    LINING    - Cambrella<br>
                                    TOE       - Steel<br>
                                    SOCKS     - Moulded Antistatic<br>
                                    INSOLE    - Kevlar<br>
                                    SOLE      - Double/Single Density
                              
                              </span>
                           </div>
                        </div>
                     </div>
                     <!--Project box End--> 
                     <!--Project box Start-->
                     <div class="col-md-4 col-sm-6">
                        <div class="pro-box">
                            <img src="images/safety-products/shoe9.jpg" alt="">
                           <h5>EN ISO 20345:2011</h5>
                           <div class="pro-hover">
                              <h6>ART NO - COMMANDO-331</h6>
                              <span><br>ART NO.   - COMMANDO-331<br>
                                        STANDARD  - S1P<br>
                                        UPPER     - Buff Barton<br>
                                        COLOUR    - Black<br>
                                        LINING    - Cambrella<br>
                                        TOE       - Steel<br>
                                        SOCKS     - Moulded Antistatic<br>
                                        INSOLE    - Kevlar<br>
                                        SOLE      - Double/Single Density
                              
                              </span>
                           </div>
                        </div>
                     </div>
                     <!--Project box End--> 
                     <!--Project box Start-->
                     <div class="col-md-4 col-sm-6">
                        <div class="pro-box">
                            <img src="images/safety-products/shoee10.jpg" alt="">
                           <h5>EN ISO 20345:2011</h5>
                           <div class="pro-hover">
                              <h6>ART NO - COMMANDO-332</h6>
                             <span><br>ART NO.   - COMMANDO-332<br>
                                       STANDARD  - S1P<br>
                                       UPPER     - Buff Barton<br>
                                       COLOUR    - Black<br>
                                       LINING    - Cambrella<br>
                                       TOE       - Steel<br>
                                       SOCKS     - Moulded Antistatic<br>
                                       INSOLE    - Kevlar<br>
                                       SOLE      - Double/Single Density
                              
                              </span>
                           </div>
                        </div>
                     </div>
                     <!--Project box End--> 
                     <!--Project box Start-->
                     <div class="col-md-4 col-sm-6">
                        <div class="pro-box">
                            <img src="images/safety-products/shoee11.jpg" alt="">
                           <h5>EN ISO 20345:2011</h5>
                           <div class="pro-hover">
                              <h6>ART NO.   - COMMANDO-332</h6>
                             <span><br>ART NO.   - COMMANDO-332<br>
                                    STANDARD  - S1P<br>
                                    UPPER     - Buff Barton<br>
                                    COLOUR    - Black/White/Brown<br>
                                    LINING    - Cambrella<br>
                                    TOE       - Steel<br>
                                    SOCKS     - Moulded Antistatic<br>
                                    INSOLE    - Kevlar<br>
                                    SOLE      - Double/Single Density
                              
                              </span>
                           </div>
                        </div>
                     </div>
                     <!--Project box End--> 
                     <!--Project box Start-->
                     <div class="col-md-4 col-sm-6">
                        <div class="pro-box">
                            <img src="images/safety-products/shoe12.jpg" alt="">
                           <h5>EN ISO 20345:2011</h5>
                           <div class="pro-hover">
                              <h6>ART NO.   - COMMANDO-02</h6>
                              <span><br>ART NO.   - COMMANDO-02<br>
                                    STANDARD  - S1P<br>
                                    UPPER     - Buff Barton<br>
                                    COLOUR    - Ten<br>
                                    LINING    - Cambrella<br>
                                    TOE       - Steel<br>
                                    SOCKS     - Moulded Antistatic<br>
                                    INSOLE    - Kevlar<br>
                                    SOLE      - Double/Single Density
                              
                              </span> 
                           </div>
                        </div>
                     </div>
                     <!--Project box End--> 
                     <!--Project box Start-->
                     <div class="col-md-4 col-sm-6">
                        <div class="pro-box">
                            <img src="images/safety-products/shoe13.jpg" alt="">
                           <h5>EN ISO 20345:2011</h5>
                           <div class="pro-hover">
                              <h6>ART NO.   - GLADSTONE</h6>
                             <span><br>ART NO.   - GLADSTONE<br>
                                    STANDARD  - S1P<br>
                                    UPPER     - Buff Barton<br>
                                    COLOUR    - Black/Grey<br>
                                    LINING    - Cambrella<br>
                                    TOE       - Steel<br>
                                    SOCKS     - Moulded Antistatic<br>
                                    INSOLE    - Kevlar<br>
                                    SOLE      - Double/Single Density
                              
                              </span>
                           </div>
                        </div>
                     </div>
                     <!--Project box End--> 
                     <!--Project box Start-->
                     <div class="col-md-4 col-sm-6">
                        <div class="pro-box">
                            <img src="images/safety-products/shoe14.jpg" alt="">
                           <h5>EN ISO 20345:2011</h5>
                           <div class="pro-hover">
                              <h6>ART NO.   - PERTH</h6>
                             <span><br>ART NO.   - PERTH<br>
                                    STANDARD  - S1P<br>
                                    UPPER     - Buff Barton<br>
                                    COLOUR    - Black<br>
                                    LINING    - Cambrella<br>
                                    TOE       - Steel<br>
                                    SOCKS     - Moulded Antistatic<br>
                                    INSOLE    - Kevlar<br>
                                    SOLE      - Double/Single Density
                              
                              </span>
                           </div>
                        </div>
                     </div>
                     <!--Project box End--> 
                  </div>
               </div>
            </div>
                     </section>
        
        
        
        <section class="wf100 p80">
            <div class="container">
                <div class="row">
                    <h3>TECHNICAL ADVANTAGE</h3>
                </div><br>
                
                <div class="Table">
   
    <div class="Heading">
        <div class="Cell">
            <span>CATEGORY</span>
        </div>
        <div class="Cell">
            <span>FEATURES</span>
        </div>
        <div class="Cell">
            <p>ADVANTAGES</p>
        </div>
    </div>
    <div class="Row">
        <div class="Cell">
            <span>Weight</span>
        </div>
        <div class="Cell">
            <span>Light weight approx,850gms + or - 50gms/pr, for size 8</span>
        </div>
        <div class="Cell">
            <span>Light Weight shoes ensure comfort to feet and peace of mind.Reduce fatigue and improves productivity</span>
        </div>
    </div>
    <div class="Row">
        <div class="Cell">
            <span>Life</span>
        </div>
        <div class="Cell">
            <span>Average life span of 1-2 years</span>
        </div>
        <div class="Cell">
            <span>Good value for your money and easy maintenance</span>
        </div>
    </div>
                    <div class="Row">
        <div class="Cell">
            <span>Sole</span>
        </div>
        <div class="Cell">
            <span>PU Sole made with PUF technology</span>
        </div>
        <div class="Cell">
            <span>Durability is very high.PU soles are oil/acid resistant,highly flexible,light weight & shock absorbing.DIP ensures high quality sole with better bonding and integrated molecular structure for long use </span>
        </div>
    </div>
                    <div class="Row">
        <div class="Cell">
            <span>Wear and Tear</span>
        </div>
        <div class="Cell">
            <span>Wear & Tear resistant and can withstand rough terrain</span>
        </div>
        <div class="Cell">
            <span>Being a PU sole made using the DIP machine,the abrasion resistance is superior than other soling material.</span>
        </div>
    </div>
                    <div class="Row">
        <div class="Cell">
            <span>Production Technique</span>
        </div>
        <div class="Cell">
            <span>Made by direct injection process with robotic technology using state of the art german machines,ensuring uniformity of the product.</span>
        </div>
        <div class="Cell">
            <span>A technologically advanced superior product. used by most MNCs and safety companies who are concered about employee safety.</span>
        </div>
    </div>
                    <div class="Row">
        <div class="Cell">
            <span>Toe</span>
        </div>
        <div class="Cell">
            <span>High quality steel/light weight non-metal toe as per the latest standards.</span>
        </div>
        <div class="Cell">
            <span>Perfect fitting and comfort around the toe area. Can withstand up to 200 joules of impact</span>
        </div>
    </div>
                    <div class="Row">
        <div class="Cell">
            <span>Penetration Resistance</span>
        </div>
        <div class="Cell">
            <span>High quality steel/light weight kevlar midsole provides penetration resistance.</span>
        </div>
        <div class="Cell">
            <span>Ensure resistance against penetration of nails and sharp objects.</span>
        </div>
    </div>
                    <div class="Row">
        <div class="Cell">
            <span>Upper Leather</span>
        </div>
        <div class="Cell">
            <span>Finest quality genuine leather for uppers with thickness of 2mm</span>
        </div>
        <div class="Cell">
            <span>Water resistant(optional),breathable & durable shoes which make the feet odour free and cool by absorbing sweat. in house tannery provide high quality din/non din leather.</span>
        </div>
    </div>
                    <div class="Row">
        <div class="Cell">
            <span>Comfort</span>
        </div>
        <div class="Cell">
            <span>Liberty shoes are made with "Humantech technology" which ensures maximum comfort.Research & development is a continous process at liberty</span>
        </div>
        <div class="Cell">
            <span>Humantech approach to shoe making ensures basic comfort.considering wide range of ergonomic features of human anatomy.  </span>
        </div>
    </div>
                    <div class="Row">
        <div class="Cell">
            <span>National & International certifications</span>
        </div>
        <div class="Cell">
            <span>Our shoes have successfully passed stringment quality checks laid down by national(BIS,DGMS) & International(EN,SS-513 etc)agencies.</span>
        </div>
        <div class="Cell">
            <span>Improves the confidence of customer.</span>
        </div>
    </div>
                </div><br>
            </div>
                </section>
                           <section class="wf100 p80">
            <div class="container">
               
                                <div class="row">
                    <h3>COMMERICAL ADVANTGAE </h3>
                </div><br>
                
                <div class="Table">
   
    <div class="Heading">
        <div class="Cell">
            <span>CATEGORY</span>
        </div>
        <div class="Cell">
            <span>FEATURES</span>
        </div>
        <div class="Cell">
            <p>ADVANTAGES</p>
        </div>
    </div>
    <div class="Row">
        <div class="Cell">
            <span>Reliability</span>
        </div>
        <div class="Cell">
            <span>Organised sector company</span>
        </div>
        <div class="Cell">
            <span>Bearing the ISO 9001-2008 tag lends consistency of credibility to the product.</span>
        </div>
    </div>
    <div class="Row">
        <div class="Cell">
            <span>Infrastructure</span>
        </div>
        <div class="Cell">
            <span>State-of-the-art infrastructure among the best in the world.</span>
        </div>
        <div class="Cell">
            <span>Supported by six state-of-the-art plants in-house tabbery,full fledged testing laboratory,highly skilled manpower,ERP/SAP system in operation.</span>
        </div>
    </div>
                    <div class="Row">
        <div class="Cell">
            <span>Customization</span>
        </div>
        <div class="Cell">
            <span>Customer friendly organisation.</span>
        </div>
        <div class="Cell">
            <span>Customer needs are tailor made for bulk requirement.</span>
        </div>
    </div>
                    <div class="Row">
        <div class="Cell">
            <span>After Sales Services</span>
        </div>
        <div class="Cell">
            <span>Channel partners are located in different parts of the globe.</span>
        </div>
        <div class="Cell">
            <span>To provide customer service at their door step,enhancing the supply and after sale services.</span>
        </div>
    </div>
                    <div class="Row">
        <div class="Cell">
            <span>Supply Chain Management </span>
        </div>
        <div class="Cell">
            <span>Customer Satisfication.</span>
        </div>
        <div class="Cell">
            <span>Improves transparency,timely delivery and eliminates stockouts at the customer warehouse by using supply chain management tools.</span>
        </div>
    </div>
                    <div class="Row">
        <div class="Cell">
            <span>Pricing</span>
        </div>
        <div class="Cell">
            <span>Resonable and affordable prices. Value for Money</span>
        </div>
        <div class="Cell">
            <span>Our valued customers happily and willingly pay for the product.Price does not matter against the safety of human life.</span>
        </div>
    </div>
                </div><br>
                
                <div class="row">
                        <div class="col-md-12">
                            <div class="legacy-trust">
                                <h3>PERFORMANCE AND TECHNICAL FEATURES</h3>
                            </div>
                        </div>
                </div><br>
                    
                         <div class="row">
                        <div class="col-md-6">
                            <div class="legacy-trust">
                            <h5>Antistatic</h5>
                            <img src="images/safety-products/001.jpg" alt="" /><span>Shoe having electrical resistance between 100K Oms to 1000M Ohms</span>
                        </div>
                    </div>
                             
                             <div class="col-md-6">
                            <div class="legacy-trust">
                            <h5>Water resistant</h5>
                            <img src="images/safety-products/002.jpg" alt="" /><span>As per the latest standards</span>
                        </div>
                    </div>
                         </div>
                
                <div class="row">
                        <div class="col-md-6">
                            <div class="legacy-trust">
                            <h5>Feather Light</h5>
                            <img src="images/safety-products/003.jpg" alt="" /><span>PU sole provides optimum comfort and reduces fatigue.</span>
                        </div>
                    </div>
                             
                             <div class="col-md-6">
                            <div class="legacy-trust">
                            <h5>Antiskid</h5>
                            <img src="images/safety-products/004.jpg" alt="" /><span>Slip resistance as per latest industry standards</span>
                        </div>
                    </div>
                         </div>
                
                <div class="row">
                        <div class="col-md-6">
                            <div class="legacy-trust">
                            <h5>Electrical Insulation</h5>
                            <img src="images/safety-products/005.jpg" alt="" /><span>Footwear(EIF/HVI) resistant to high voltage.</span>
                        </div>
                    </div>
                             
                             <div class="col-md-6">
                            <div class="legacy-trust">
                            <h5>Toe Cap</h5>
                            <img src="images/safety-products/006.jpg" alt="" /><span>High quality steel/fibre toe to withstand energy upto 200J.</span></span>
                        </div>
                    </div>
                         </div>
                
                <div class="row">
                        <div class="col-md-6">
                            <div class="legacy-trust">
                            <h5>Shock Absorber</h5>
                            <img src="images/safety-products/007.jpg" alt="" /><span>Being a PU sole,it absorbs the shocks/jerks during work.</span>
                        </div>
                    </div>
                             
                             <div class="col-md-6">
                            <div class="legacy-trust">
                            <h5>Mono/Dual Density</h5>
                            <img src="images/safety-products/008.jpg" alt="" /><span>PU/PU, PU/TPU,PU RUBBER.</span>
                        </div>
                    </div>
                         </div>
                
                <div class="row">
                        <div class="col-md-6">
                            <div class="legacy-trust">
                            <h5>Abrasian Resistant</h5>
                            <img src="images/safety-products/009.jpg" alt="" /><span>Being a PU outsole,the abrasion resistance is much better than other common outsole materials.</span>
                        </div>
                    </div>
                             
                             <div class="col-md-6">
                            <div class="legacy-trust">
                            <h5>All Terrain/Weather Shoe </h5>
                            <img src="images/safety-products/010.jpg" alt="" /><span>Shoe is customized as per requirement</span>
                        </div>
                    </div>
                         </div>
                
                <div class="row">
                        <div class="col-md-6">
                            <div class="legacy-trust">
                            <h5>ESD</h5>
                            <img src="images/safety-products/011.jpg" alt="" /><span>Safety shoe with electrical resistance ranging from 100k Ohms to 100m Ohms</span>
                        </div>
                    </div>
                             
                             <div class="col-md-6">
                            <div class="legacy-trust">
                            <h5>Genuine Leather</h5>
                            <img src="images/safety-products/013.jpg" alt="" /><span>Din/Non Din made by inhouse tannery.</span>
                        </div>
                    </div>
                         </div>
                
                <div class="row">
                        <div class="col-md-6">
                            <div class="legacy-trust">
                            <h5>Oil/Acid resistant</h5>
                            <img src="images/safety-products/014.jpg" alt="" /><span>Outsole offers protection on work surface prone to oil spills sole is oil and acid resistant</span>
                        </div>
                    </div>
                             
                             <div class="col-md-6">
                            <div class="legacy-trust">
                            <h5>Heat resistant</h5>
                            <img src="images/safety-products/015.jpg" alt="" /><span>specially designed light weight rubber outsole withstands temperatures upto 300 degree c</span>
                        </div>
                    </div>
                         </div>
                
                <div class="row">
                        <div class="col-md-6">
                            <div class="legacy-trust">
                            <h5>Penetration Resistant</h5>
                            <img src="images/safety-products/016.jpg" alt="" /><span>PWithstands nail penetration upto 1100N by providing steel/Kevlar midsole.</span>
                        </div>
                    </div>
                             
                             <div class="col-md-6">
                            <div class="legacy-trust">
                            <h5>Fabric</h5>
                            <img src="images/safety-products/017.jpg" alt="" /><span>Superior quality fabric not only enhances the look and comfort,but can also be customized for water repellent property</span>
                        </div>
                    </div>
                         </div>
                
                <div class="row">
                        <div class="col-md-6">
                            <div class="legacy-trust">
                            <h5>100% Metal free</h5>
                            <img src="images/safety-products/019.jpg" alt="" /><span>Footwear entirely made of metal free components which are light weight and do not set off security alarms.</span>
                        </div>
                    </div>
                             
                             <div class="col-md-6">
                            <div class="legacy-trust">
                            <h5>Cold Protection</h5>
                            <img src="images/safety-products/020.jpg" alt="" /><span>Footwear specially designed and produced to ensure perfect thermal comfort for the wearers in low temperature.</span>
                        </div>
                    </div>
                         </div>
                
                <div class="row">
                        <div class="col-md-6">
                            <div class="legacy-trust">
                            <h5>Ankle Protection</h5>
                            <img src="images/safety-products/021.jpg" alt="" /><span>Footwear protecting ankle against accident impacts</span>
                        </div>
                    </div>
                             
                             <div class="col-md-6">
                            <div class="legacy-trust">
                            <h5>Metatarsal Protection</h5>
                            <img src="images/safety-products/022.jpg" alt="" /><span>Footwear against metatarsal protection upto 100J impact which protects the instep against the accident fall of heavy objects. </span>
                        </div>
                    </div>
                         </div>
            
            </div>
        </section>
        </div>
    
        
          

            <!--Causes End--> 
             <?php include'footer.php'; ?>
        
        
    </body>
</html>

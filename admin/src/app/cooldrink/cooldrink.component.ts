...
export class CooldrinkComponent implements OnInit {
  ...
  getDrinks(): void {
    this.httpClient.get<any>('http://localhost/mushak/onlinebuy/api/v1/get_drinks')
      .subscribe(
        (res) => {
...
export class CoolDrinkForm {
  ...
  constructor(
    ...
    this.httpClient.get('http://localhost/mushak/onlinebuy/api/v1/get_unit').subscribe(
      (res) => {
...
export class CooldrinkDelete {
  ...
  confirmDelete() {
    ...
    this.loading = true;
    this.httpClient.get('http://localhost/mushak/onlinebuy/api/v1/delete_record/cooldrink/cooldrink_id=' + this.cooldrink_id).subscribe(
      (res) => {
        this.loading = false;
        if (res["result"]["error"] === false) {
          this.dialogRef.close(true);

...
export class SnacksComponent implements OnInit {
  ...
  getSnacks(): void {
    this.httpClient.get<any>('http://www.lemonandshadow.com/threelevel/api/v1/get_foods')
      .subscribe(
        (res) => {
...
export class SnacksForm {
  ...
  onSubmit() {
    ...
    if (this.id != 0) {
      ...
      url = 'update_record/fooditems/id = ' + this.id;
    } else {
      formData.append('name', this.snacksform.value.name);
      formData.append('amount', this.snacksform.value.amount);
      formData.append('unit_id', this.snacksform.value.unit_id);
...
export class SnacksDelete {
  ...
  confirmDelete() {
    ...
    this.loading = true;
    this.httpClient.get('http://www.lemonandshadow.com/threelevel/api/v1/delete_record/fooditems/id=' + this.id).subscribe(
      (res) => {
        this.loading = false;
        if (res["result"]["error"] === false) {
          this.dialogRef.close(true);

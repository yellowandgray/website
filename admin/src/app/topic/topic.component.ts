...
getChapter(): void {
        this.httpClient.get<any>('http://localhost/mushak/feringo/api/v1/get_chapter_by_subject/'+this.topicForm.value.subject_id)
        .subscribe(
                (res)=>{
                    this.chapter = res["result"]["data"];
              },
...
export class TopicDelete {
  ...
  confirmDelete() {
      ...
      this.loading = true;
      this.httpClient.get('http://localhost/mushak/feringo/api/v1/delete_record/topic/topic_id='+this.topic_id).subscribe(
          (res)=>{
                this.loading = false;
                if(res["result"]["error"] === false) {
                    this.dialogRef.close(true);

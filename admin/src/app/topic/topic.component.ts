...
this.chapter = [];
    this.selectedchapind = ev.index;
        this.httpClient.get<any>('http://localhost/mushak/feringo_beta/api/v1/get_chapter_by_subject/'+this.subject[ev.index].subject_id)
        .subscribe(
                (res)=>{
...
if(typeof this.chapter[ev.index] !== 'undefined') {
        this.httpClient.get<any>('http://localhost/mushak/feringo_beta/api/v1/get_topic_by_chapter/'+this.chapter[ev.index].chapter_id)
        .subscribe(
                (res)=>{
...
getChapter(): void {
        this.httpClient.get<any>('http://localhost/mushak/feringo_beta/api/v1/get_chapter_by_subject/'+this.topicForm.value.subject_id)
        .subscribe(
                (res)=>{
                    this.chapter = res["result"]["data"];
              },
...
export class TopicDelete {
  ...
  confirmDelete() {
      ...
      this.loading = true;
      this.httpClient.get('http://localhost/mushak/feringo_beta/api/v1/delete_record/topic/topic_id='+this.topic_id).subscribe(
          (res)=>{
                this.loading = false;
                if(res["result"]["error"] === false) {
                    this.dialogRef.close(true);

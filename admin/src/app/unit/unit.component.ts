...
export class UnitComponent implements OnInit {
  ...
  getUnit(): void {
    this.httpClient.get<any>('http://www.lemonandshadow.com/threelevel/api/v1/get_unit')
      .subscribe(
        (res) => {
...
export class UnitDelete {
  ...
  confirmDelete() {
    ...
    this.loading = true;
    this.httpClient.get('http://www.lemonandshadow.com/threelevel/api/v1/delete_record/unit/unit_id=' + this.unit_id).subscribe(
      (res) => {
        this.loading = false;
        if (res["result"]["error"] === false) {
          this.dialogRef.close(true);

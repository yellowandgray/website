...
export class UserComponent implements OnInit {
    ...
    getuser(): void {
        this.httpClient.get<any>('http://localhost/microview/megana/api/v1/get_student')
        .subscribe(
                (res)=>{
...
export class UserDelete {
  ...
  confirmDelete() {
      ...
      this.loading = true;
      this.httpClient.get('http://localhost/microview/megana/api/v1/delete_record/student_register/student_register_id='+this.student_register_id).subscribe(
          (res)=>{
                this.loading = false;
                if(res["result"]["error"] === false) {
                    this.dialogRef.close(true);

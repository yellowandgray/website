...
export class UserForm {
  ...
  fileProgress(fileInput: any, name: string, path: string) {
    ...
    this.httpClient.post('http://localhost/project/examhorse/api/v1/upload_file', formData).subscribe(
      (res) => {
        this.loading = false;
        if (res["result"]["error"] === false) {
...
export class UserDelete {
  ...
  confirmDelete() {
    ...
    this.loading = true;
    this.httpClient.get('http://localhost/project/examhorse/api/v1/delete_record/student_register/student_register_id=' + this.student_register_id).subscribe(
      (res) => {
        this.loading = false;
        if (res["result"]["error"] === false) {
          this.dialogRef.close(true);

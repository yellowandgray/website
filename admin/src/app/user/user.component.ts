...
export class UserComponent implements OnInit {
  ...
  getuser(): void {
    this.httpClient.get<any>('http://localhost/mushak/feringo_beta/api/v1/get_student/'+this.sortdata)
      .subscribe(
        (res) => {
  ...
  openResult(sid, name): void {
      this.httpClient.get<any>('http://localhost/mushak/feringo_beta/api/v1/get_student_result/'+sid)
      .subscribe(
        (res) => {
...
export class UserDelete {
  ...
  confirmDelete() {
    ...
    this.loading = true;
    this.httpClient.get('http://localhost/mushak/feringo_beta/api/v1/delete_record/student_register/student_register_id=' + this.student_register_id).subscribe(
      (res) => {
        this.loading = false;
        if (res["result"]["error"] === false) {
          this.dialogRef.close(true);

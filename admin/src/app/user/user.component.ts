...
export class UserComponent implements OnInit {
  ...
  getUser(): void {
    this.httpClient.get<any>('http://localhost/project/three-levan/api/v1/get_user')
      .subscribe(
        (res) => {
...
export class UserDelete {
  ...
  confirmDelete() {
    ...
    this.loading = true;
    this.httpClient.get('http://localhost/project/three-levan/api/v1/delete_record/user/user_id=' + this.user_id).subscribe(
      (res) => {
        this.loading = false;
        if (res["result"]["error"] === false) {
          this.dialogRef.close(true);

...
export class BookComponent implements OnInit {
  ...
  getBook(): void {
    this.httpClient.get<any>('http://localhost/project/exam-horse-sample/api/v1/get_book')
      .subscribe(
        (res) => {
...
export class BookDelete {
  ...
  confirmDelete() {
      ...
      this.loading = true;
      this.httpClient.get('http://localhost/project/exam-horse-sample/api/v1/delete_record/book/book_id='+this.book_id).subscribe(
          (res)=>{
                this.loading = false;
                if(res["result"]["error"] === false) {
                    this.dialogRef.close(true);

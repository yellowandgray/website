...
export class CouponComponent implements OnInit {
  ...
  getCoupon(): void {
    this.httpClient.get<any>('http://localhost/microview/fresche/api/v1/get_coupon')
      .subscribe(
        (res) => {
...
export class CouponDelete {
  ...
  confirmDelete() {
    ...
    this.loading = true;
    this.httpClient.get('http://localhost/microview/fresche/api/v1/delete_record/coupon/coupon_id=' + this.coupon_id).subscribe(
      (res) => {
        this.loading = false;
        if (res["result"]["error"] === false) {
          this.dialogRef.close(true);

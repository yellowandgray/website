...
export class BannerComponent implements OnInit {
  ...
  getBanner(): void {
    this.httpClient.get<any>('http://lemonandshadow.com/threelevel/api/v1/get_banner')
      .subscribe(
        (res) => {
...
export class BannerDelete {
  ...
  confirmDelete() {
    ...
    this.loading = true;
    this.httpClient.get('http://lemonandshadow.com/threelevel/api/v1/delete_record/banner/banner_id=' + this.banner_id).subscribe(
      (res) => {
        this.loading = false;
        if (res["result"]["error"] === false) {
          this.dialogRef.close(true);

...
export class BannerComponent implements OnInit {
  ...
  getBanner(): void {
    this.httpClient.get<any>('http://www.lemonandshadow.com/threelevel/api/v1/get_banner')
      .subscribe(
        (res) => {
...
export class BannerForm {
  ...
  onSubmit() {
    ...
    if (this.banner_id != 0) {
      ...
      formData.append('sub_title', this.bannerform.value.sub_title);
...
export class BannerDelete {
  ...
  confirmDelete() {
    ...
    this.loading = true;
    this.httpClient.get('http://www.lemonandshadow.com/threelevel/api/v1/delete_record/banner/banner_id=' + this.banner_id).subscribe(
      (res) => {
        this.loading = false;
        if (res["result"]["error"] === false) {
          this.dialogRef.close(true);

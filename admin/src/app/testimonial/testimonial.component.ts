...
export class TestimonialComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    getTestimonial(): void {
     this.httpClient.get<any>('http://localhost/project/fresche/api/v1/get_testimonial')
     .subscribe(
             (res)=>{
...
export class TestimonialDelete {
  ...
  confirmDelete() {
      ...
      this.loading = true;
      this.httpClient.get('http://localhost/project/fresche/api/v1/delete_record/testimonial/testimonial_id='+this.testimonial_id).subscribe(
          (res)=>{
                this.loading = false;
                if(res["result"]["error"] === false) {
                    this.dialogRef.close(true);

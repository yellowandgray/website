...
export class SubjectForm {
  ...
  onSubmit() {
    ...
    if (this.subject_id != 0) {
      ...
      formData.append('standard', this.subjectForm.value.standard);
...
export class SubjectDelete {
  ...
  confirmDelete() {
    ...
    this.loading = true;
    this.httpClient.get('http://2dotask.com/yellowandgray/feringo_beta/api/v1/delete_record/subject/subject_id=' + this.subject_id).subscribe(
      (res) => {
        this.loading = false;
        if (res["result"]["error"] === false) {
          this.dialogRef.close(true);

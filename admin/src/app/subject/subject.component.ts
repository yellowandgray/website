...
export class SubjectComponent implements OnInit {
  ...
  image_url: string = 'http://localhost/microview/megana/api/v1/';
  getsubject(): void {
...
export class SubjectForm {
  ...
  fileProgress(fileInput: any, name: string, path: string) {
    ...
    this.httpClient.post('http://localhost/microview/megana/api/v1/upload_file', formData).subscribe(
      (res) => {
        this.loading = false;
        if (res["result"]["error"] === false) {
...
export class SubjectDelete {
  ...
  confirmDelete() {
    ...
    this.loading = true;
    this.httpClient.get('http://localhost/microview/megana/api/v1/delete_record/subject/subject_id=' + this.subject_id).subscribe(
      (res) => {
        this.loading = false;
        if (res["result"]["error"] === false) {
          this.dialogRef.close(true);

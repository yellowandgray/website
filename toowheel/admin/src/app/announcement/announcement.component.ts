...
export class AnnouncementComponent implements OnInit {
  ...
  image_url: string = 'http://www.toowheel.com/toowheel/api/v1/';
     ...
     this.httpClient.get<any>('http://www.toowheel.com/toowheel/api/v1/get_announcement/'+this.club_id)
     .subscribe(
             (res)=>{
...
export class AnnouncementDelete {
  ...
  confirmDelete() {
      ...
      this.loading = true;
      this.httpClient.get('http://www.toowheel.com/toowheel/api/v1/delete_record/announcement/announcement_id='+this.announcement_id).subscribe(
          (res)=>{
                this.loading = false;
                if(res["result"]["error"] === false) {
                    this.dialogRef.close(true);

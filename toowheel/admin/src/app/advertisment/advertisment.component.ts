...
export class AdvertismentComponent implements OnInit {
  ...
  this.httpClient.get<any>('api/v1/get_advertisment')
  .subscribe(
          (res)=>{
...
export class AdvertismentDelete {
  ...
  confirmDelete() {
      ...
      this.loading = true;
      this.httpClient.get('api/v1/delete_record/advertisement/advertisement_id='+this.advertisement_id).subscribe(
          (res)=>{
                this.loading = false;
                if(res["result"]["error"] === false) {
                    this.dialogRef.close(true);

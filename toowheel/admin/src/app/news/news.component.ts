...
export class NewsForm {
    ...
    getCategory(): void {
       this.loading = true;
          this.httpClient.get('http://www.toowheel.com/toowheel/api/v1/get_'+this.newsForm.value.type+'_category').subscribe(
              (res)=>{
                this.loading = false;
                if(res["result"]["error"] === false) {
    ...
    getClub(): void {
       this.loading = true;
          this.httpClient.get('http://www.toowheel.com/toowheel/api/v1/get_club_by_category/'+this.newsForm.value.category_id).subscribe(
              (res)=>{
                this.loading = false;
                if(res["result"]["error"] === false) {
...
export class NewsGalleryForm {
    ...
    getImages(){
        this.httpClient.get('http://www.toowheel.com/toowheel/api/v1/get_news_gallery_by_news/'+this.news_id).subscribe(
              (res)=>{
                if(res["result"]["error"] === false) {
                    this.result = res["result"]["data"];
                }else{
  ...
  submitPhotos(fileInput: any) {
      ...
      this.httpClient.post('http://www.toowheel.com/toowheel/api/v1/insert_news_gallery', formData).subscribe(
          (res)=>{
                this.loading = false;
                if(res["result"]["error"] === false) {
  ...
  deleteGallery(id) {
      ...
      this.loading = true;
      this.httpClient.get('http://www.toowheel.com/toowheel/api/v1/delete_record/news_gallery/news_gallery_id='+id).subscribe(
          (res)=>{
                this.loading = false;
                if(res["result"]["error"] === false) {
                    this.getImages();
...
export class NewsDelete {
  ...
  confirmDelete() {
      ...
      this.loading = true;
      this.httpClient.get('http://www.toowheel.com/toowheel/api/v1/delete_record/news/news_id='+this.news_id).subscribe(
          (res)=>{
                this.loading = false;
                if(res["result"]["error"] === false) {
                    this.dialogRef.close(true);

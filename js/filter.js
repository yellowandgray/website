...
app.filter('image_url', function () {
    return function (input) {
        var image = 'http://eshine.myarisenshine.com/v1/';
        return image + input;
    };
});
...
app.filter('templateparent', function ($rootScope) {
    return function (input) {
        var found = false;
        var name = '-';
        angular.forEach($rootScope.contents, function (val, key) {
            if (!found && parseInt(val.ID) === parseInt(input)) {
                name = val.name;
            }
        });
...
app.filter('trustAsResourceUrl', ['$sce', function ($sce) {
        return function (val) {
            return $sce.trustAsResourceUrl('http://eshine.myarisenshine.com/v1/' + val);
        };
    }]);
...
app.filter('image', function () {
    return function (input) {
        if (input && input !== 0) {
            return 'http://eshine.myarisenshine.com/v1/' + input.split(',')[1];
        }
    };
});